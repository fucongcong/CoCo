...
{   
    ...
    {   
        ...
        if (!is_dir($apiDir."/src/main/java/{$groupname}/{$module}/api")) {
            $this->error('module:{$module}不存在...初始化失败，请先执行 generate:module');
        }

        if (!is_dir($apiDir."/src/main/java/{$groupname}/{$module}/api/controller")) {
            mkdir($daoDir."/src/main/java/{$group}/{$module}/api/controller", 0755, true);
        }

        if (file_exists($apiDir."/controller/".ucfirst($name)."Controller.java")) {
            $this->error(ucfirst($name)."Controller.java 已存在");
        }

        $data = $this->getFile("api/Controller.java.tpl");
        file_put_contents($apiDir."/controller/".ucfirst($name)."Controller.java", $data);
